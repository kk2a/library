...
template <algebra::CommutativeMonoid M> struct SegmentTree2D {
    ...
    M prod(int i1, int j1, int i2, int j2) {
  ...
  private:
    ...
    void updatei(int i, int j) { d[i][j] = M::op(d[i * 2][j], d[i * 2 + 1][j]); }

    void updatej(int i, int j) { d[i][j] = M::op(d[i][j * 2], d[i][j * 2 + 1]); }

    M inner_prod(int i, int j1, int j2) {
        M res = M::unit();
        for (; j1 < j2; j1 >>= 1, j2 >>= 1) {
            if (j1 & 1) res = M::op(res, d[i][j1++]);
            if (j2 & 1) res = M::op(d[i][--j2], res);
        }
        return res;
    }
};
