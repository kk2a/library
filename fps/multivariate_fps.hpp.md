...
data:
  ...
  bundledCode: "#line 1 \"fps/multivariate_fps.hpp\"\n\n\n\n#include <algorithm>\n\
    ...
    \ long pow_mod(long long x, long long n, long long m) {\n    if (m == 1) return\
    ...
    \        tmp[i + j - (i + j >= k ? k : 0)] +=\n                            a[i][ii]\
    \ * b[j][ii];\n                    }\n                }\n                for (int\
    \ i = 0; i < k; i++) c[i][ii] = tmp[i], tmp[i] = mint{0};\n            }\n   \
    \     };\n\n        // reference:\n        // https://nyaannyaan.github.io/library/ntt/multivariate-multiplication.hpp\n\
    ...
    \ = inv2;\n        for (int d = 1; d < n; d <<= 1) {\n            std::vector<fps>\
    \ a(k, fps(2 * d)), b(k, fps(2 * d)),\n                c(k, fps(2 * d));\n   \
    \         for (int i = 0; i < std::min((int)f.size(), 2 * d); i++)\n         \
    \       a[chi[i]][i] = f[i];\n            for (int i = 0; i < d; i++) b[chi[i]][i]\
    \ = g[i];\n            for (auto &x : a) x.but();\n            for (auto &x :\
    \ b) x.but();\n            naive_and_dot(a, b, c);\n            for (auto &x :\
    \ c) {\n                x.ibut();\n                x *= i2d;\n            }\n\
    ...
    \ (((*this) * f0inv).log() * e).exp() * coef;\n        }\n        int n = f.size();\n\
    \        long long base_sum = 0;\n        for (auto &b : base) base_sum += b -\
    \ 1;\n        if (e > base_sum) return mfps(base, fps(n));\n        mfps res(base,\
    \ fps(n)), a(*this);\n        res.f[0] = 1;\n        while (e) {\n           \
    \ if (e & 1) res *= a;\n            if (e >>= 1) a *= a;\n        }\n        return\
    \ res;\n    }\n};\n\ntemplate <typename mint>\nstd::vector<mint> MultivariateFormalPowerSeries<mint>::_inv\
    \ = {0, 1};\n\n} // namespace kk2\n\n\n"
