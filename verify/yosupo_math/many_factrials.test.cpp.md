...
data:
  ...
  - code: "#line 1 \"verify/yosupo_math/many_factrials.test.cpp\"\n// competitive-verifier:\
      ...
      \ g = sample_point_shift<FPS>(f, mint(1 << i), 3 << i);\n            const auto\
      \ get = [&](int j) {\n                return j < (1 << i) ? f[j] : g[j - (1\
      \ << i)];\n            };\n            f.resize(2 << i);\n            for (int\
      \ j = 0; j < 2 << i; j++) {\n                f[j] = get(2 * j) * get(2 * j +\
      \ 1) * ((2 * j + 1) << i);\n            }\n        }\n\n        if (BLOCK_NUM\
      \ > BLOCK_SIZE) {\n            std::vector<mint> g =\n                sample_point_shift<FPS>(f,\
      \ mint(BLOCK_SIZE), BLOCK_NUM - BLOCK_SIZE);\n            std::move(std::begin(g),\
      \ std::end(g), std::back_inserter(f));\n        } else f.resize(BLOCK_NUM);\n\
  ...
  requiredBy: []
  testcases:
  - elapsed: 3.1029166639999914
    environment: g++
    memory: 51.444
    name: example_00
    status: AC
  - elapsed: 3.2870664170000055
    environment: g++
    memory: 55.004
    name: many_bits_00
    status: AC
  - elapsed: 3.33608646899998
    environment: g++
    memory: 62.16
    name: many_bits_01
    status: AC
  - elapsed: 3.375517765000012
    environment: g++
    memory: 60.52
    name: many_bits_02
    status: AC
  - elapsed: 3.306339511999994
    environment: g++
    memory: 52.796
    name: many_bits_03
    status: AC
  - elapsed: 3.2983992860000058
    environment: g++
    memory: 54.404
    name: many_bits_04
    status: AC
  - elapsed: 3.1250978190000183
    environment: g++
    memory: 37.104
    name: near_half_00
    status: AC
  - elapsed: 3.110451712000014
    environment: g++
    memory: 37.104
    name: near_half_01
    status: AC
  - elapsed: 3.124693301999997
    environment: g++
    memory: 37.104
    name: near_max_00
    status: AC
  - elapsed: 3.1158907740000075
    environment: g++
    memory: 37.104
    name: near_max_01
    status: AC
  - elapsed: 3.379299197999984
    environment: g++
    memory: 54.996
    name: random_00
    status: AC
  - elapsed: 3.4464899570000114
    environment: g++
    memory: 64.856
    name: random_01
    status: AC
