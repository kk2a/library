...
data:
  ...
  - code: "// competitive-verifier: PROBLEM https://judge.yosupo.jp/problem/ordered_set\n\
      ...
      \  return 0;\n}\n"
    name: default
  - code: "#line 1 \"verify/yosupo_ds/ds_ordered_set_max_right.test.cpp\"\n// competitive-verifier:\
      \ PROBLEM https://judge.yosupo.jp/problem/ordered_set\n\n#line 1 \"bbst/red_black_tree.hpp\"\
      \n\n\n\n#include <cassert>\n#include <memory>\n#include <string>\n#include <utility>\n\
      #include <vector>\n\n#line 1 \"bbst/base/red_black_tree_base.hpp\"\n\n\n\n#line\
      \ 9 \"bbst/base/red_black_tree_base.hpp\"\n\n#line 1 \"others/vector_pool.hpp\"\
      ...
      \ (pos >= free_ptrs.size()) { expand(); }\n        return free_ptrs[pos++];\n\
      \    }\n\n    inline void free(T *ptr) { free_ptrs[--pos] = ptr; }\n\n    void\
      \ clear() {\n        pos = 0;\n        size_t idx = 0;\n        for (auto &chunk\
      \ : chunks) {\n            for (size_t i = 0; i < CHUNK_SIZE; i++) { free_ptrs[idx++]\
      \ = &chunk[i]; }\n        }\n    }\n\n    void reserve(int n) {\n        size_t\
      \ needed_chunks = (n + CHUNK_SIZE - 1) / CHUNK_SIZE;\n        size_t current_chunks\
      ...
      \          for (size_t i = 0; i < CHUNK_SIZE; ++i) { free_ptrs[base_idx + i]\
      ...
      \ l and l <= r and r <= size(t));\n        auto [t1, t2, t3] = split3(t, l,\
      \ r);\n        if (t2) t2->is_rev ^= 1;\n        t = merge(merge(t1, t2), t3);\n\
      \    }\n\n    template <typename... Args> void push_front(NodePtr &t, Args...\
      ...
      \u30A4\u30F3\u30BF\uFF08\u5B58\u5728\u3057\u306A\u3044\u5834\u5408\u306Fnullptr\uFF09\
      \n     *\n     * @pre 0 <= l <= size(t)\n     * @pre g(s_unit()) == true\n \
      \    */\n    template <class G> bb_result max_right(NodePtr &t, int l, const\
      ...
      \     *\n     * - k = 0 \u307E\u305F\u306F (k \u2260 0 \u304B\u3064 g(prod(k-1,\
      \ r)) = false)\n     *\n     * @tparam G \u8FF0\u8A9E\u95A2\u6570\u306E\u578B\
      \u3002bool operator()(S)\u3092\u6301\u3064\u5FC5\u8981\u304C\u3042\u308A\u307E\
      \u3059\n     * @param t \u63A2\u7D22\u5BFE\u8C61\u306E\u6728\uFF08\u53C2\u7167\
      \u6E21\u3057\u3001\u64CD\u4F5C\u5F8C\u306B\u5FA9\u5143\u3055\u308C\u307E\u3059\
      ...
      \u30CE\u30FC\u30C9\u3078\u306E\u30DD\u30A4\u30F3\u30BF\uFF08\u5B58\u5728\u3057\
      \u306A\u3044\u5834\u5408\u306Fnullptr\uFF09\n     *\n     * @pre 0 <= r <= size(t)\n\
      \     * @pre g(s_unit()) == true\n     */\n    template <class G> bb_result\
      \ min_left(NodePtr &t, int r, const G &g) {\n        assert(0 <= r and r <=\
      \ size(t));\n        assert(g(s_unit()));\n        auto [t1, t2] = split(t,\
      \ r);\n        if (!t1) {\n            t = merge(t1, t2);\n            return\
      \ {r, s_unit(), nullptr};\n        }\n        if (g(t1->val)) {\n          \
      ...
      \ as_root(NodePtr t) {\n        if (!t) return t;\n        t->is_red = false;\n\
      \        return t;\n    }\n};\n\n} // namespace rbtree\n\n} // namespace kk2\n\
      \n\n#line 1 \"bbst/base/red_black_tree_node.hpp\"\n\n\n\n#line 1 \"type_traits/io.hpp\"\
      \n\n\n\n#include <concepts>\n#include <fstream>\n#include <istream>\n#include\
      ...
      \            }\n        }\n\n        os << \"\\n]\\n\";\n    }\n};\n\n} // namespace\
      \ rbtree\n\n} // namespace kk2\n\n\n#line 12 \"bbst/red_black_tree.hpp\"\n\n\
      namespace kk2 {\n\nnamespace rbtree {\n\ntemplate <class M> struct RedBlackTree\
      \ : RedBlackTreeBase<RedBlackTree<M>, RedBlackTreeNode<M>> {\n    using base\
      \ = RedBlackTreeBase<RedBlackTree<M>, RedBlackTreeNode<M>>;\n    using base::RedBlackTreeBase;\n\
      \    using base::s_op;\n    using base::size;\n    using typename base::NodePtr;\n\
      \    using typename base::S;\n\n    NodePtr update(NodePtr t) {\n        t->count\
      \ = size(t->left) + size(t->right) + (t->left == nullptr);\n        t->rank\
      \ = t->left ? t->left->rank + !t->left->is_red : 1;\n        t->val = (t->left\
      \ ? s_op(t->left->val, t->right->val) : t->val);\n        return t;\n    }\n\
      ...
      \ const S &b) { return (a > b ? a = b, 1 : 0); }\n\n\n#line 6 \"verify/yosupo_ds/ds_ordered_set_max_right.test.cpp\"\
      \nusing namespace std;\n\nint main() {\n    int n, q;\n    kin >> n >> q;\n\
      \    vc<kk2::monoid::Hold<int>> a(n);\n    kin >> a;\n    kk2::RedBlackTree<kk2::monoid::Hold<int>>\
      \ rb(2 * (n + q));\n    auto root = rb.build(a);\n    rep(q) {\n        int\
      \ t, x;\n        kin >> t >> x;\n        if (t == 0) {\n            auto [k,\
      \ a, b] = rb.max_right(root, 0, [&](auto t) { return t.is_unit or t.a < x; });\n\
      ...
      \ kout << rb.get(root, x) << \"\\n\";\n        }\n        if (t == 3) {\n  \
      \          kout << rb.max_right(root, 0, [&](auto t) { return t.is_unit or t.a\
      \ <= x; }).s << \"\\n\";\n        }\n        if (t == 4) {\n            auto\
      \ [k, a, b] = rb.max_right(root, 0, [&](auto t) { return t.is_unit or t.a <=\
      \ x; });\n            if (a.is_unit) kout << -1 << \"\\n\";\n            else\
      \ kout << a << \"\\n\";\n        }\n        if (t == 5) {\n            auto\
      \ [k, a, b] = rb.max_right(root, 0, [&](auto t) { return t.is_unit or t.a <\
      \ x; });\n            if (b) kout << b->val << \"\\n\";\n            else kout\
  ...
  requiredBy: []
  testcases:
  - elapsed: 0.002256843999987268
    environment: g++
    memory: 4.02
    name: example_00
    status: AC
  - elapsed: 0.0022871580000014546
    environment: g++
    memory: 3.988
    name: example_01
    status: AC
  - elapsed: 0.22346496499994828
    environment: g++
    memory: 50.684
    name: max_random_00
    status: AC
  - elapsed: 0.13590728799999852
    environment: g++
    memory: 50.636
    name: max_random_01
    status: AC
  - elapsed: 0.1454901780000455
    environment: g++
    memory: 50.648
    name: max_random_02
    status: AC
  - elapsed: 0.09412028500003089
    environment: g++
    memory: 50.644
    name: max_random_03
    status: AC
  - elapsed: 0.12634298600005422
    environment: g++
    memory: 50.64
    name: max_random_04
    status: AC
  - elapsed: 0.12188023599998132
    environment: g++
    memory: 50.636
    name: max_random_05
    status: AC
  - elapsed: 0.14641018699995811
    environment: g++
    memory: 50.644
    name: max_random_06
    status: AC
  - elapsed: 0.14015032199995403
    environment: g++
    memory: 50.644
    name: max_random_07
    status: AC
  - elapsed: 0.24434033299996827
    environment: g++
    memory: 50.644
    name: max_random_08
    status: AC
  - elapsed: 0.16091031499996689
    environment: g++
    memory: 50.588
    name: max_random_09
    status: AC
  - elapsed: 0.1571995400000219
    environment: g++
    memory: 50.548
    name: max_random_10
    status: AC
  - elapsed: 0.0970492390000004
    environment: g++
    memory: 50.644
    name: max_random_11
    status: AC
  - elapsed: 0.12407874500001981
    environment: g++
    memory: 50.64
    name: max_random_12
    status: AC
  - elapsed: 0.1566402720000042
    environment: g++
    memory: 50.516
    name: max_random_13
    status: AC
  - elapsed: 0.1695413559999679
    environment: g++
    memory: 50.676
    name: max_random_14
    status: AC
  - elapsed: 0.1705922300000111
    environment: g++
    memory: 50.632
    name: max_random_15
    status: AC
  - elapsed: 0.8468887399999971
    environment: g++
    memory: 101.492
    name: max_random_16
    status: AC
  - elapsed: 0.6657461120000221
    environment: g++
    memory: 101.46
    name: max_random_17
    status: AC
  - elapsed: 1.3635478440000384
    environment: g++
    memory: 101.208
    name: max_random_18
    status: AC
  - elapsed: 1.4212804039999583
    environment: g++
    memory: 101.456
    name: max_random_19
    status: AC
  - elapsed: 0.5577764219999608
    environment: g++
    memory: 101.456
    name: max_random_20
    status: AC
  - elapsed: 0.4277176850000046
    environment: g++
    memory: 101.46
    name: max_random_21
    status: AC
  - elapsed: 0.4149722050000264
    environment: g++
    memory: 101.404
    name: max_random_22
    status: AC
  - elapsed: 0.44726053700003376
    environment: g++
    memory: 101.504
    name: max_random_23
    status: AC
  - elapsed: 0.9654732559999957
    environment: g++
    memory: 101.4
    name: max_random_24
    status: AC
  - elapsed: 0.6945071929999926
    environment: g++
    memory: 101.372
    name: max_random_25
    status: AC
  - elapsed: 1.2058461429999738
    environment: g++
    memory: 101.46
    name: max_random_26
    status: AC
  - elapsed: 1.4151919010000142
    environment: g++
    memory: 101.3
    name: max_random_27
    status: AC
  - elapsed: 0.5515095000000088
    environment: g++
    memory: 101.464
    name: max_random_28
    status: AC
  - elapsed: 0.508751736000022
    environment: g++
    memory: 101.388
    name: max_random_29
    status: AC
  - elapsed: 0.48968275800001493
    environment: g++
    memory: 101.464
    name: max_random_30
    status: AC
  - elapsed: 0.48814230400000724
    environment: g++
    memory: 101.444
    name: max_random_31
    status: AC
  - elapsed: 0.06768986000002997
    environment: g++
    memory: 50.668
    name: small_00
    status: AC
  - elapsed: 0.07238052499997139
    environment: g++
    memory: 50.568
    name: small_01
    status: AC
  - elapsed: 0.07637410399996725
    environment: g++
    memory: 50.636
    name: small_02
    status: AC
