...
data:
  ...
  - files: []
    type: Verified with
  dependsOn:
  - convolution/convolution_subset.hpp
  - modint/mont.hpp
  ...
  - code: "// competitive-verifier: PROBLEM https://judge.yosupo.jp/problem/subset_convolution\n\
      \n#include \"../../convolution/convolution_subset.hpp\"\n#include \"../../modint/mont.hpp\"\
      \n#include \"../../template/template.hpp\"\nusing namespace std;\n\nint main()\
      \ {\n    int n;\n    kin >> n;\n    vc<kk2::mont998> a(1 << n), b(1 << n);\n\
      \    kin >> a >> b;\n    kout << kk2::convolution_subset(a, b) << kendl;\n\n\
      \    return 0;\n}\n"
    name: default
  - code: "#line 1 \"verify/yosupo_convolution/convolution_subset.test.cpp\"\n// competitive-verifier:\
      \ PROBLEM https://judge.yosupo.jp/problem/subset_convolution\n\n#line 1 \"convolution/convolution_subset.hpp\"\
      \n\n\n\n#include <cassert>\n#include <functional>\n#include <vector>\n\nnamespace\
      \ kk2 {\n\ntemplate <class FPS, class mint = typename FPS::value_type>\nFPS\
      \ convolution_subset(FPS &a, const FPS &b) {\n    int n = int(size(a)); // ==\
      \ int(size(b)\n    if (!n) return {};\n    int h = __builtin_ctz(n);\n\n   \
      \ assert((n & (-n)) == n);\n    std::vector<int> chi(n, 0);\n    for (int i\
      \ = 1; i < n; i++) chi[i] = chi[i - (i & -i)] + 1;\n    std::vector<std::vector<mint>>\
      \ f(n, std::vector<mint>(h + 1)), g(n, std::vector<mint>(h + 1));\n    for (int\
      \ i = 0; i < n; i++) {\n        f[i][chi[i]] = a[i];\n        g[i][chi[i]] =\
      ...
      \ const S &b) { return (a > b ? a = b, 1 : 0); }\n\n\n#line 6 \"verify/yosupo_convolution/convolution_subset.test.cpp\"\
      \nusing namespace std;\n\nint main() {\n    int n;\n    kin >> n;\n    vc<kk2::mont998>\
  ...
  requiredBy: []
  testcases:
  - elapsed: 0.0026101389999979574
    environment: g++
    memory: 3.752
    name: example_00
    status: AC
  - elapsed: 15.496914347
    environment: g++
    memory: 265.452
    name: hack01_00
    status: AC
  - elapsed: 15.647262104999996
    environment: g++
    memory: 265.576
    name: max_random_00
    status: AC
  - elapsed: 15.639618373999994
    environment: g++
    memory: 265.608
    name: max_random_01
    status: AC
  - elapsed: 15.650540013000011
    environment: g++
    memory: 265.488
    name: max_random_02
    status: AC
  - elapsed: 15.612593705999984
    environment: g++
    memory: 265.608
    name: random_00
    status: AC
  - elapsed: 0.002650471000009702
    environment: g++
    memory: 3.764
    name: random_01
    status: AC
  - elapsed: 0.006936002999992752
    environment: g++
    memory: 4.016
    name: random_02
    status: AC
  - elapsed: 0.0020910720000131278
    environment: g++
    memory: 3.784
    name: small_00
    status: AC
  - elapsed: 0.0022736279999833187
    environment: g++
    memory: 3.72
    name: small_01
    status: AC
  - elapsed: 0.0021007119999865154
    environment: g++
    memory: 3.696
    name: small_02
    status: AC
