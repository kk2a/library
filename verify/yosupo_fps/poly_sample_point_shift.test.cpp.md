...
data:
  ...
  - code: "// competitive-verifier: PROBLEM\n// https://judge.yosupo.jp/problem/shift_of_sampling_points_of_polynomial\n\
      ...
      \ {\n    int n, m;\n    kk2::mont998 c;\n    kin >> n >> m >> c;\n    vc<kk2::mont998>\
      \ a(n);\n    kin >> a;\n    kout << kk2::sample_point_shift<FPS>(a, c, m) <<\
      \ kendl;\n\n    return 0;\n}\n"
    name: default
  - code: "#line 1 \"verify/yosupo_fps/poly_sample_point_shift.test.cpp\"\n// competitive-verifier:\
      \ PROBLEM\n// https://judge.yosupo.jp/problem/shift_of_sampling_points_of_polynomial\n\
      ...
      \ < m; i++) {\n        ret[i] = cur * dh[k + i];\n        cur *= t + i + 1;\n\
      \        cur *= h[i];\n    }\n    return ret;\n}\n\n} // namespace kk2\n\n\n\
      #line 5 \"verify/yosupo_fps/poly_sample_point_shift.test.cpp\"\n\n#line 1 \"\
      fps/fps_ntt_friendly.hpp\"\n\n\n\n#line 5 \"fps/fps_ntt_friendly.hpp\"\n#include\
      ...
      \ = this->size();\n        return (derived().diff() * derived().inv(deg)).pre(deg\
      \ - 1).integral();\n    }\n    FPS sparse_log(int deg = -1) const {\n      \
      \  if (deg == -1) deg = this->size();\n        std::vector<std::pair<int, mint>>\
      \ fs;\n        for (int i = 1; i < int(this->size()); i++) {\n            if\
      \ ((*this)[i] != mint(0)) fs.emplace_back(i, (*this)[i]);\n        }\n     \
      \   ivta::set_upper(deg);\n\n        FPS g(deg);\n        for (int k = 0; k\
      \ < deg - 1; k++) {\n            for (auto &[j, fj] : fs) {\n              \
      \  if (k < j) break;\n                int i = k - j;\n                g[k +\
      \ 1] -= g[i + 1] * fj * (i + 1);\n            }\n            g[k + 1] *= ivta::inv(k\
      \ + 1);\n            if (k + 1 < int(this->size())) g[k + 1] += (*this)[k +\
      \ 1];\n        }\n\n        return g;\n    }\n\n    template <class T> FPS dense_pow(T\
      ...
      \ mint>> fs;\n        for (int i = 1; i < int(this->size()); i++) {\n      \
      \      if ((*this)[i] != mint(0)) fs.emplace_back(i, (*this)[i]);\n        }\n\
      \n        int mod = mint::getmod();\n        static std::vector<mint> inv{1,\
      \ 1};\n        int now = inv.size();\n        inv.resize(std::max(now, deg +\
      \ 1));\n        for (int i = now; i <= deg; i++) inv[i] = -inv[mod % i] * (mod\
      \ / i);\n\n        FPS g(deg);\n        if (deg) g[0] = 1;\n        for (int\
      ...
      \ const S &b) { return (a > b ? a = b, 1 : 0); }\n\n\n#line 9 \"verify/yosupo_fps/poly_sample_point_shift.test.cpp\"\
      \nusing namespace std;\n\nusing FPS = kk2::FPSNTT<kk2::mont998>;\n\nint main()\
      \ {\n    int n, m;\n    kk2::mont998 c;\n    kin >> n >> m >> c;\n    vc<kk2::mont998>\
      \ a(n);\n    kin >> a;\n    kout << kk2::sample_point_shift<FPS>(a, c, m) <<\
      \ kendl;\n\n    return 0;\n}\n"
  ...
  requiredBy: []
  testcases:
  - elapsed: 0.21651772400002756
    environment: g++
    memory: 11.064
    name: N_1_00
    status: AC
  - elapsed: 1.571498785000017
    environment: g++
    memory: 21.128
    name: c_0_00
    status: AC
  - elapsed: 0.0026659170000016275
    environment: g++
    memory: 3.656
    name: example_00
    status: AC
  - elapsed: 0.0022350640000468047
    environment: g++
    memory: 3.656
    name: example_01
    status: AC
  - elapsed: 3.2312683119999974
    environment: g++
    memory: 34.316
    name: max_random_00
    status: AC
  - elapsed: 3.252954580999983
    environment: g++
    memory: 34.316
    name: max_random_01
    status: AC
  - elapsed: 3.2421006040000293
    environment: g++
    memory: 34.312
    name: max_random_02
    status: AC
  - elapsed: 3.2180143799999996
    environment: g++
    memory: 34.312
    name: max_random_03
    status: AC
  - elapsed: 0.04396834400000671
    environment: g++
    memory: 4.028
    name: medium_random_00
    status: AC
  - elapsed: 0.04393951700001253
    environment: g++
    memory: 4.028
    name: medium_random_01
    status: AC
  - elapsed: 0.04389915500001962
    environment: g++
    memory: 4.072
    name: medium_random_02
    status: AC
  - elapsed: 0.04403349499995102
    environment: g++
    memory: 4.076
    name: medium_random_03
    status: AC
  - elapsed: 0.002617659999998523
    environment: g++
    memory: 3.608
    name: small_random_00
    status: AC
  - elapsed: 0.002344609000033415
    environment: g++
    memory: 3.804
    name: small_random_01
    status: AC
  - elapsed: 0.0023303860000396526
    environment: g++
    memory: 3.6
    name: small_random_02
    status: AC
  - elapsed: 0.0023338299999977608
    environment: g++
    memory: 3.812
    name: small_random_03
    status: AC
  - elapsed: 0.05405641499999092
    environment: g++
    memory: 5.388
    name: type0_random_00
    status: AC
  - elapsed: 0.05669027900000856
    environment: g++
    memory: 5.552
    name: type0_random_01
    status: AC
  - elapsed: 0.009410378000040964
    environment: g++
    memory: 4.168
    name: type0_random_02
    status: AC
  - elapsed: 0.06376651800002264
    environment: g++
    memory: 5.764
    name: type0_random_03
    status: AC
  - elapsed: 3.054518127999984
    environment: g++
    memory: 32.076
    name: type1_random_00
    status: AC
  - elapsed: 3.0250403449999794
    environment: g++
    memory: 30.952
    name: type1_random_01
    status: AC
  - elapsed: 0.1779219800000078
    environment: g++
    memory: 5.74
    name: type1_random_02
    status: AC
  - elapsed: 1.6128806500000223
    environment: g++
    memory: 22.652
    name: type1_random_03
    status: AC
  - elapsed: 3.0866329669999573
    environment: g++
    memory: 30.712
    name: type2_random_00
    status: AC
  - elapsed: 3.1338851859999863
    environment: g++
    memory: 32.24
    name: type2_random_01
    status: AC
  - elapsed: 0.8279284220000136
    environment: g++
    memory: 12.356
    name: type2_random_02
    status: AC
  - elapsed: 1.5574947629999656
    environment: g++
    memory: 20.368
    name: type2_random_03
    status: AC
  - elapsed: 1.6203730409999935
    environment: g++
    memory: 20.548
    name: type3_random_00
    status: AC
  - elapsed: 3.076217530000008
    environment: g++
    memory: 31.068
    name: type3_random_01
    status: AC
  - elapsed: 1.1242601899999727
    environment: g++
    memory: 11.06
    name: type3_random_02
    status: AC
  - elapsed: 1.545751284000005
    environment: g++
    memory: 20.212
