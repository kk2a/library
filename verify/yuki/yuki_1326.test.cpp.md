...
data:
  ...
  - code: "// competitive-verifier: PROBLEM https://yukicoder.me/problems/no/1326\n\
      ...
      \ --res;\n        kout << res / 2 << \"\\n\";\n    }\n\n    return 0;\n}\n"
    name: default
  - code: "#line 1 \"verify/yuki/yuki_1326.test.cpp\"\n// competitive-verifier: PROBLEM\
      ...
      \n\n\n\n#line 6 \"graph/tree/block_cut_tree.hpp\"\n\n#line 1 \"graph/bcc.hpp\"\
      ...
      graph/tree/heavy_light_decomposition.hpp\"\n\nnamespace kk2 {\n\ntemplate <graph::UndirectedGraph\
      \ G> struct HeavyLightDecomposition {\n\n    G &g;\n    int root, id;\n    std::vector<int>\
      \ sz, in, out, head, par, dep, edge_idx;\n\n    // e.id\u306Fedges\u306Eindex\u3067\
      ...
      \ out[u]); }\n\n    template <typename F> void path_query(int u, int v, bool\
      ...
      \           if constexpr (requires { g.swap_edges(v, i, j); }) g.swap_edges(v,\
      \ i, j);\n            else std::swap(g[v][i], g[v][j]);\n        };\n      \
      \  auto dfs_sz = [&](auto self, int now) -> void {\n            sz[now] = 1;\n\
      \            for (int i = 0; i < (int)g[now].size(); ++i) {\n              \
      \  if (g[now][i].to == par[now]) {\n                    if (g[now].size() >=\
      \ 2 and g[now][i].id == g[now][0].id) {\n                        swap_edges(now,\
      \ i, 1);\n                    } else {\n                        continue;\n\
      \                    }\n                }\n                const int child =\
      \ g[now][i].to;\n                par[child] = now;\n                dep[child]\
      \ = dep[now] + 1;\n                self(self, child);\n                sz[now]\
      \ += sz[child];\n                if (sz[child] > sz[g[now][0].to]) swap_edges(now,\
      \ i, 0);\n            }\n        };\n        dfs_sz(dfs_sz, root);\n\n     \
      ...
      \ in[head[u]]);\n            u = par[head[u]];\n        }\n        if (u !=\
      \ v) res.emplace_back(in[u], in[v] + 1);\n        return res;\n    }\n\n   \
      \ // (u, v]\n    std::vector<std::pair<int, int>> descend(int u, int v) const\
      \ {\n        if (u == v) return {};\n        if (head[u] == head[v]) return\
      \ {std::make_pair(in[u] + 1, in[v])};\n        auto res = descend(u, par[head[v]]);\n\
      ...
      \ const S &b) { return (a > b ? a = b, 1 : 0); }\n\n\n#line 7 \"verify/yuki/yuki_1326.test.cpp\"\
      \nusing namespace std;\n\nint main() {\n    int n, m;\n    kin >> n >> m;\n\
      \    kk2::AdjList g(n, m, kin, 1);\n\n    kk2::BlockCutTree bct(g);\n    kk2::HeavyLightDecomposition\
      \ hld(bct.forest);\n\n    int q;\n    kin >> q;\n\n    rep(q) {\n        int\
      \ x, y;\n        kin >> x >> y;\n        --x, --y;\n        if (bct.comp_v[x]\
  ...
  requiredBy: []
  testcases:
  - elapsed: 0.0025482829999958767
    environment: g++
    memory: 3.824
    name: size_1__sample_1
    status: AC
  - elapsed: 0.0023180099999962067
    environment: g++
    memory: 3.808
    name: size_1_gen_1
    status: AC
  - elapsed: 0.0026146259999961785
    environment: g++
    memory: 3.836
    name: size_1_gen_2
    status: AC
  - elapsed: 0.002720168999999828
    environment: g++
    memory: 3.852
    name: size_1_gen_3
    status: AC
  - elapsed: 0.0029345970000065336
    environment: g++
    memory: 3.984
    name: size_1_gen_4
    status: AC
  - elapsed: 0.002106072000003678
    environment: g++
    memory: 3.684
    name: size_1_hand_1
    status: AC
  - elapsed: 0.0021226700000056553
    environment: g++
    memory: 3.816
    name: size_1_hand_2
    status: AC
  - elapsed: 0.005416448000005403
    environment: g++
    memory: 4.2
    name: size_2_gen_1
    status: AC
  - elapsed: 0.005486622999995916
    environment: g++
    memory: 4.216
    name: size_2_gen_2
    status: AC
  - elapsed: 0.004872484000003396
    environment: g++
    memory: 4.068
    name: size_2_gen_3
    status: AC
  - elapsed: 0.005989509999992038
    environment: g++
    memory: 4.344
    name: size_2_gen_4
    status: AC
  - elapsed: 0.005962569999994116
    environment: g++
    memory: 4.264
    name: size_2_gen_tree_1
    status: AC
  - elapsed: 0.22905661299999736
    environment: g++
    memory: 26.252
    name: size_3_gen_1
    status: AC
  - elapsed: 0.22896716600000389
    environment: g++
    memory: 26.216
    name: size_3_gen_2
    status: AC
  - elapsed: 0.22773445200000708
    environment: g++
    memory: 26.224
    name: size_3_gen_3
    status: AC
  - elapsed: 0.22508170400000438
    environment: g++
    memory: 26.036
    name: size_3_gen_4
    status: AC
  - elapsed: 0.21243699599999388
    environment: g++
    memory: 24.956
    name: size_3_gen_5
    status: AC
  - elapsed: 0.1679941749999898
    environment: g++
    memory: 20.212
    name: size_3_gen_6
    status: AC
  - elapsed: 0.14506575699999757
    environment: g++
    memory: 16.944
    name: size_3_gen_7
    status: AC
  - elapsed: 0.1109351990000107
    environment: g++
    memory: 17.74
    name: size_3_gen_big_cycle_1
    status: AC
  - elapsed: 0.23879893500000549
    environment: g++
    memory: 34.72
    name: size_3_gen_large_ans_1
    status: AC
  - elapsed: 0.24280371099999343
    environment: g++
    memory: 34.772
    name: size_3_gen_line_1
    status: AC
  - elapsed: 0.2795717800000119
    environment: g++
    memory: 38.336
    name: size_3_gen_line_clique_1
    status: AC
  - elapsed: 0.1866868330000102
    environment: g++
    memory: 22.504
    name: size_3_gen_star_1
    status: AC
  - elapsed: 0.23005592900000238
    environment: g++
    memory: 26.252
    name: size_3_gen_tree_1
    status: AC
