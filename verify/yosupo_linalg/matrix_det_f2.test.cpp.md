...
data:
  ...
  - type_traits/io.hpp
  embedded:
  - code: "// competitive-verifier: PROBLEM https://judge.yosupo.jp/problem/matrix_det_mod_2\n\
      \n#include \"../../matrix/matrix_F2.hpp\"\n#include \"../../template/template.hpp\"\
      \nusing namespace std;\n\nint main() {\n    int n;\n    kin >> n;\n    kk2::MatrixF2\
      \ a(n, n);\n    kout << a.input(kin).det() << \"\\n\";\n\n    return 0;\n}\n"
    name: default
  - code: "#line 1 \"verify/yosupo_linalg/matrix_det_f2.test.cpp\"\n// competitive-verifier:\
      ...
      \ static int BLOCK_MASK = BLOCK_SIZE - 1;\n    constexpr static UInt ONE = 1;\n\
      \    int n;\n    std::vector<UInt> block;\n\n    DynamicBitSet(int n_ = 0, bool\
      \ x = 0) : n(n_) {\n        UInt val = x ? -1 : 0;\n        block.assign((n\
      ...
      \    n += rhs.n;\n        return *this;\n    }\n\n    T combine_top(const T\
      \ &rhs) const { return T(*this).inplace_combine_top(rhs); }\n\n    T &inplace_combine_bottom(const\
      \ T &rhs) {\n        block.resize((n + rhs.n + BLOCK_SIZE - 1) >> BLOCK_SIZE_LOG);\n\
      ...
      \            n += rhs.n;\n            return *this;\n        }\n        int\
      \ start = BLOCK_SIZE - (rhs.n & BLOCK_MASK);\n        UInt start_mask = (ONE\
      \ << start) - 1;\n        UInt end_mask = ~start_mask;\n        for (int i =\
      \ ((n + BLOCK_SIZE - 1) >> BLOCK_SIZE_LOG) - 1; i >= 0; --i) {\n           \
      \ UInt x = block[i];\n            block[i + (rhs.n >> BLOCK_SIZE_LOG)] |= (x\
      \ & start_mask) << (BLOCK_SIZE - start);\n            if (i + (rhs.n >> BLOCK_SIZE_LOG)\
      \ + 1 < (n + rhs.n + BLOCK_SIZE - 1) >> BLOCK_SIZE_LOG)\n                block[i\
      \ + (rhs.n >> BLOCK_SIZE_LOG) + 1] |= (x & end_mask) >> start;\n        }\n\
      ...
      \ &lhs, const T &rhs) { return T(lhs) |= rhs; }\n\n    friend T operator^(const\
      ...
      \ i++) _mat[i] &= rhs._mat[i];\n        return *this;\n    }\n\n    mat &operator*=(const\
      \ mat &rhs) {\n        assert(_w == rhs._h);\n        std::vector<DynamicBitSet>\
      \ res(_h, DynamicBitSet(rhs._w));\n        for (int i = 0; i < _h; i++) {\n\
      \            for (int j = 0; j < _w; j++) {\n                if (_mat[i][j])\
      \ res[i] ^= rhs._mat[j];\n            }\n        }\n        _w = rhs._w;\n \
      \       _mat = std::move(res);\n        return *this;\n    }\n\n    /**\n  \
      \   * @brief \u6383\u304D\u51FA\u3057\n     * @tparam RREF \u884C\u7C21\u7D04\
      \u5316\u884C\u5217\u306B\u3059\u308B\u304B\u3069\u3046\u304B\n     * @tparam\
      \ EARLY_TERMINATE \u30D5\u30EB\u30E9\u30F3\u30AF\u3067\u306A\u3044\u3053\u3068\
      \u304C\u78BA\u5B9A\u3057\u305F\u3068\u304D\u306B\u5373\u6253\u3061\u5207\u308B\
      \u304B\u3069\u3046\u304B\n     * @param wr \u5217\u304C[0, wr)\u306E\u7BC4\u56F2\
      \u3060\u3051\u3067\u6383\u304D\u51FA\u3059\uFF0E\u6307\u5B9A\u304C\u306A\u3044\
      \u306A\u3089\u5168\u4F53\u3067\u6383\u304D\u51FA\u3059\uFF0E\n     * @return\
      \ int \u30E9\u30F3\u30AF\uFF0EEARLY_TERMINATE\u304Ctrue\u306E\u3068\u304D\u306B\
      \uFF0C\u30D5\u30EB\u30E9\u30F3\u30AF\u3067\u306A\u3044\u3053\u3068\u304C\u78BA\
      \u5B9A\u3057\u305F\u3089-1\u3092\u8FD4\u3059\n     */\n    template <bool RREF\
      \ = true, bool EARLY_TERMINATE = false> int sweep(int wr = -1) {\n        if\
      \ (wr == -1) wr = _w;\n        int r = 0;\n        for (int i = 0; i < wr; i++)\
      \ {\n            if (r >= _h) break;\n            int pivot = -1;\n        \
      \    for (int j = r; j < _h; j++) {\n                if (_mat[j][i]) {\n   \
      \                 pivot = j;\n                    break;\n                }\n\
      ...
      \ idx(_w, -1), step(r, -1);\n        for (int i = 0, j = 0, now = 0; j < _w;\
      \ j++) {\n            if (i == r or !bool(ab[i][j])) idx[j] = now, res[1 + now++][j]\
      \ = 1;\n            else res[0][j] = ab[i][_w], step[i++] = j;\n        }\n\
      \        for (int i = 0; i < r; i++) {\n            for (int j = 0; j < _w;\
      \ j++) {\n                if (idx[j] != -1) res[idx[j] + 1][step[i]] = ab[i][j];\n\
      \            }\n        }\n        return res;\n    }\n\n    int rank() const\
      \ { return mat(*this).sweep<false, false>(); }\n\n    bool det() const {\n \
      \       assert(_h == _w);\n        return mat(*this).sweep<false, true>() !=\
      ...
      \ const S &b) { return (a > b ? a = b, 1 : 0); }\n\n\n#line 5 \"verify/yosupo_linalg/matrix_det_f2.test.cpp\"\
      \nusing namespace std;\n\nint main() {\n    int n;\n    kin >> n;\n    kk2::MatrixF2\
      \ a(n, n);\n    kout << a.input(kin).det() << \"\\n\";\n\n    return 0;\n}\n"
    name: bundled
  isFailed: false
  ...
  requiredBy: []
  testcases:
  - elapsed: 0.0016892349999579892
    environment: g++
    memory: 3.928
    name: example_00
    status: AC
  - elapsed: 0.0014947089999850505
    environment: g++
    memory: 3.908
    name: example_01
    status: AC
  - elapsed: 0.3848959430000036
    environment: g++
    memory: 7.672
    name: lowrank_max_random_00
    status: AC
  - elapsed: 0.6496893369999839
    environment: g++
    memory: 7.672
    name: lowrank_max_random_01
    status: AC
  - elapsed: 0.4060931370000276
    environment: g++
    memory: 7.416
    name: lowrank_max_random_02
    status: AC
  - elapsed: 0.42607083500001863
    environment: g++
    memory: 7.664
    name: lowrank_max_random_03
    status: AC
  - elapsed: 0.3763688210000282
    environment: g++
    memory: 7.288
    name: lowrank_max_random_04
    status: AC
  - elapsed: 0.8496018120000031
    environment: g++
    memory: 8.056
    name: max_random_00
    status: AC
  - elapsed: 0.8529311960000427
    environment: g++
    memory: 8.056
    name: max_random_01
    status: AC
  - elapsed: 0.85136752599999
    environment: g++
    memory: 8.052
    name: max_random_02
    status: AC
  - elapsed: 0.8554636640000126
    environment: g++
    memory: 8.056
    name: max_random_03
    status: AC
  - elapsed: 0.9174034950000305
    environment: g++
    memory: 8.056
    name: max_random_04
    status: AC
  - elapsed: 0.9305175400000394
    environment: g++
    memory: 8.056
    name: max_random_05
    status: AC
  - elapsed: 0.01933861899999556
    environment: g++
    memory: 3.988
    name: middle_00
    status: AC
  - elapsed: 0.0030226300000322226
    environment: g++
    memory: 4.036
    name: middle_01
    status: AC
  - elapsed: 0.0018280479999930321
    environment: g++
    memory: 3.908
    name: middle_02
    status: AC
  - elapsed: 0.002887015999988307
    environment: g++
    memory: 3.892
    name: middle_03
    status: AC
  - elapsed: 0.011736437999957161
    environment: g++
    memory: 4.108
    name: middle_04
    status: AC
  - elapsed: 0.028671041999984936
    environment: g++
    memory: 4.124
    name: middle_05
    status: AC
  - elapsed: 0.34897966199997654
    environment: g++
    memory: 7.668
    name: perm_max_random_00
    status: AC
  - elapsed: 0.3465170050000097
    environment: g++
    memory: 7.664
    name: perm_max_random_01
    status: AC
  - elapsed: 0.3255550030000336
    environment: g++
    memory: 7.416
    name: perm_max_random_02
    status: AC
  - elapsed: 0.3460863130000007
    environment: g++
    memory: 7.696
    name: perm_max_random_03
    status: AC
  - elapsed: 0.31255785799999103
    environment: g++
    memory: 7.28
    name: perm_max_random_04
    status: AC
  - elapsed: 0.018388539000000037
    environment: g++
    memory: 4.1
    name: random_00
    status: AC
  - elapsed: 0.002807426000003943
    environment: g++
    memory: 3.868
    name: random_01
    status: AC
  - elapsed: 0.0017435519999935423
    environment: g++
    memory: 3.728
    name: random_02
    status: AC
  - elapsed: 0.5030870960000016
    environment: g++
    memory: 6.488
    name: random_03
    status: AC
  - elapsed: 0.30130100299999185
    environment: g++
    memory: 5.524
    name: random_04
    status: AC
  - elapsed: 0.7976909930000033
    environment: g++
    memory: 7.656
    name: random_05
    status: AC
  - elapsed: 0.001650296000036633
    environment: g++
    memory: 3.764
    name: small_00
    status: AC
  - elapsed: 0.0015150970000377129
    environment: g++
    memory: 3.768
    name: small_01
    status: AC
  - elapsed: 0.0014792420000162565
    environment: g++
    memory: 3.928
    name: small_02
    status: AC
  - elapsed: 0.0014578960000335428
    environment: g++
    memory: 3.848
    name: small_03
    status: AC
  - elapsed: 0.0014304839999681462
    environment: g++
    memory: 3.852
    name: small_04
    status: AC
  - elapsed: 0.0014205089999563825
    environment: g++
    memory: 3.864
    name: small_05
    status: AC
