...
data:
  ...
  dependsOn: []
  embedded:
  - code: "// competitive-verifier: PROBLEM https://judge.yosupo.jp/problem/matrix_rank\n\
      ...
      \n    return 0;\n}\n"
    name: default
  - code: "#line 1 \"verify/yosupo_linalg/matrix_rank.test.cpp\"\n// competitive-verifier:\
      ...
      \ _h; i++) {\n            for (int k = 0; k < _w; k++) {\n                for\
      \ (int j = 0; j < rhs._w; j++) { res[i][j] += _mat[i][k] * rhs._mat[k][j]; }\n\
      \            }\n        }\n        _w = rhs._w;\n        _mat = std::move(res);\n\
      \        return *this;\n    }\n\n    /**\n     * @brief \u6383\u304D\u51FA\u3057\
      ...
      \u3059\uFF0E\u6307\u5B9A\u304C\u306A\u3044\u306A\u3089\u5168\u4F53\u3067\u6383\
      \u304D\u51FA\u3059\uFF0E\n     * @return \u30E9\u30F3\u30AF\u3068\u884C\u5217\
      \u5F0F\u3092\u8FD4\u3059\uFF0E\n     */\n    template <bool RREF = true, bool\
      \ EARLY_TERMINATE = false>\n    std::pair<int, Field> sweep(int wr = -1) {\n\
      \        if (wr == -1) wr = _w;\n        int r = 0;\n        Field det = 1;\n\
      \        for (int i = 0; i < wr; ++i) {\n            if (r >= _h) break;\n \
      \           int pivot = r;\n            while (pivot < _h and _mat[pivot][i]\
      \ == Field(0)) ++pivot;\n            if (pivot == _h) {\n                if\
      \ constexpr (EARLY_TERMINATE) return {-1, Field(0)};\n                det =\
      \ 0;\n                continue;\n            }\n            if (r != pivot)\
      ...
      \    int n = _h;\n        for (int i = 0; i < n - 1; ++i) {\n            int\
      \ pivot = i + 1;\n            while (pivot < n and _mat[pivot][i] == Field(0))\
      \ ++pivot;\n            if (pivot == n) continue;\n            if (pivot !=\
      ...
      \ ++j) _mat[j][i + 1] *= a;\n            }\n            for (int j = i + 2;\
      \ j < n; ++j) {\n                Field a = _mat[j][i];\n                if (a\
      \ == Field(0)) continue;\n                for (int k = i; k < n; ++k) _mat[j][k]\
      \ -= _mat[i + 1][k] * a;\n                for (int k = 0; k < n; ++k) _mat[k][i\
      \ + 1] += _mat[k][j] * a;\n            }\n        }\n        return *this;\n\
      \    }\n\n    Field det() const {\n        assert(_h == _w);\n        return\
      \ mat(*this).sweep<false, true>().second;\n    }\n\n    std::optional<mat> inv()\
      \ const {\n        assert(_h == _w);\n        int n = _h;\n        mat res(*this);\n\
      \        res.inplace_combine_right(mat::unit(n));\n        int r = res.sweep<true,\
      \ true>().first;\n        if (r != n) return std::nullopt;\n        for (int\
      \ i = 0; i < n; i++) res._mat[i].erase(res[i].begin(), res[i].begin() + n);\n\
      \        res._w = n;\n        return res;\n    }\n\n    int rank() const { return\
      \ mat(*this).sweep<false, false>().first; }\n\n    mat &shrink() {\n       \
      \ while (_h and _mat.back() == std::vector<Field>(_w, Field())) _mat.pop_back(),\
      \ _h--;\n        return *this;\n    }\n\n    template <class T> mat pow(T n)\
      \ const {\n        assert(_h == _w);\n        assert(n >= 0);\n        mat mul(_mat),\
      \ res = mat::unit(_h);\n        while (n) {\n            if (n & 1) res *= mul;\n\
      \            if (n >>= 1) mul *= mul;\n        }\n        return res;\n    }\n\
      \n    std::optional<mat> solve(const mat &b) const {\n        assert(_h == b._h);\n\
      \        assert(b._w == 1);\n        mat ab = combine_right(b);\n        int\
      \ r = ab.sweep<true, false>().first;\n        if ([&]() {\n                if\
      \ (!r) return false;\n                for (int i = 0; i < _w; i++)\n       \
      \             if (ab[r - 1][i] != Field()) return false;\n                return\
      \ true;\n            }()) {\n            return std::nullopt;\n        }\n \
      \       mat res(1 + _w - r, _w);\n        std::vector<int> idx(_w, -1), step(r,\
      ...
      \ mat &rhs) {\n        assert(_w == rhs._w);\n        _mat.insert(_mat.begin(),\
      \ rhs._mat.begin(), rhs._mat.end());\n        _h += rhs._h;\n        return\
      \ *this;\n    }\n\n    mat &inplace_combine_bottom(const mat &rhs) {\n     \
      \   assert(_w == rhs._w);\n        _mat.insert(_mat.end(), rhs._mat.begin(),\
      \ rhs._mat.end());\n        _h += rhs._h;\n        return *this;\n    }\n\n\
      \    mat &inplace_combine_left(const mat &rhs) {\n        assert(_h == rhs._h);\n\
      \        for (int i = 0; i < _h; i++) _mat[i].insert(_mat[i].begin(), rhs[i].begin(),\
      \ rhs[i].end());\n        _w += rhs._w;\n        return *this;\n    }\n\n  \
      \  mat &inplace_combine_right(const mat &rhs) {\n        assert(_h == rhs._h);\n\
      \        for (int i = 0; i < _h; i++) _mat[i].insert(_mat[i].end(), rhs[i].begin(),\
      \ rhs[i].end());\n        _w += rhs._w;\n        return *this;\n    }\n\n  \
      \  mat transpose() const {\n        mat res(_w, _h);\n        for (int i = 0;\
      \ i < _h; i++) {\n            for (int j = 0; j < _w; j++) { res[j][i] = _mat[i][j];\
      \ }\n        }\n        return res;\n    }\n\n    mat combine_top(const mat\
      \ &rhs) const { return mat(*this).inplace_combine_top(rhs); }\n    mat combine_bottom(const\
      \ mat &rhs) const { return mat(*this).inplace_combine_bottom(rhs); }\n    mat\
      \ combine_left(const mat &rhs) const { return mat(*this).inplace_combine_left(rhs);\
  ...
  requiredBy: []
  testcases:
  - elapsed: 0.002322626000022865
    environment: g++
    memory: 3.84
    name: example_00
    status: AC
  - elapsed: 0.0017571779999911996
    environment: g++
    memory: 3.744
    name: example_01
    status: AC
  - elapsed: 0.0017087339999761753
    environment: g++
    memory: 3.832
    name: example_02
    status: AC
  - elapsed: 0.0016735729999481919
    environment: g++
    memory: 3.744
    name: example_03
    status: AC
  - elapsed: 0.012881005999986428
    environment: g++
    memory: 4.12
    name: hack_of_system_of_linear_00
    status: AC
  - elapsed: 0.01241880800000672
    environment: g++
    memory: 4.12
    name: hack_of_system_of_linear_01
    status: AC
  - elapsed: 0.016445315999987997
    environment: g++
    memory: 4.108
    name: hack_of_system_of_linear_02
    status: AC
  - elapsed: 0.0020528990000343583
    environment: g++
    memory: 3.852
    name: hack_of_system_of_linear_03
    status: AC
  - elapsed: 0.0027690669999742568
    environment: g++
    memory: 3.872
    name: hack_of_system_of_linear_04
    status: AC
  - elapsed: 0.012310086000013598
    environment: g++
    memory: 4.108
    name: hack_of_system_of_linear_05
    status: AC
  - elapsed: 0.12636940400000185
    environment: g++
    memory: 5.484
    name: lowrank_00
    status: AC
  - elapsed: 0.06442272300000695
    environment: g++
    memory: 5.472
    name: lowrank_01
    status: AC
  - elapsed: 0.020742919000042548
    environment: g++
    memory: 4.584
    name: lowrank_02
    status: AC
  - elapsed: 0.17465503099998614
    environment: g++
    memory: 5.484
    name: lowrank_03
    status: AC
  - elapsed: 0.005093074999990677
    environment: g++
    memory: 4.004
    name: lowrank_04
    status: AC
  - elapsed: 0.09037764499998957
    environment: g++
    memory: 4.432
    name: lowrank_05
    status: AC
  - elapsed: 0.022658106000051248
    environment: g++
    memory: 5.496
    name: max_00
    status: AC
  - elapsed: 0.02333815299999742
    environment: g++
    memory: 5.436
    name: max_01
    status: AC
  - elapsed: 0.024902244999964296
    environment: g++
    memory: 5.496
    name: max_02
    status: AC
  - elapsed: 0.5162816650000082
    environment: g++
    memory: 5.46
    name: max_03
    status: AC
  - elapsed: 0.5059757640000271
    environment: g++
    memory: 5.484
    name: max_04
    status: AC
  - elapsed: 0.5073267920000148
    environment: g++
    memory: 5.476
    name: max_05
    status: AC
  - elapsed: 0.026305610000008528
    environment: g++
    memory: 5.496
    name: max_06
    status: AC
  - elapsed: 0.02481186200003549
    environment: g++
    memory: 5.372
    name: max_07
    status: AC
  - elapsed: 0.021964281000009578
    environment: g++
    memory: 5.492
    name: max_08
    status: AC
  - elapsed: 0.25055851100000837
    environment: g++
    memory: 5.36
    name: random_00
    status: AC
  - elapsed: 0.28993692299997065
    environment: g++
    memory: 5.484
    name: random_01
    status: AC
  - elapsed: 0.0717586400000414
    environment: g++
    memory: 4.588
    name: random_02
    status: AC
  - elapsed: 0.27513634199999615
    environment: g++
    memory: 5.48
    name: random_03
    status: AC
  - elapsed: 0.005259552999973494
    environment: g++
    memory: 4.012
    name: random_04
    status: AC
  - elapsed: 0.09078694599998016
    environment: g++
    memory: 4.368
    name: random_05
    status: AC
  - elapsed: 0.001952547999962917
    environment: g++
    memory: 3.856
    name: zero_00
    status: AC
  - elapsed: 0.0016904889999977968
    environment: g++
    memory: 3.744
    name: zero_01
    status: AC
  - elapsed: 0.0016686239999899044
    environment: g++
    memory: 3.84
    name: zero_02
    status: AC
  - elapsed: 0.001677018999998836
    environment: g++
    memory: 3.724
    name: zero_03
    status: AC
  - elapsed: 0.0021307339999907526
    environment: g++
    memory: 3.82
    name: zero_04
    status: AC
