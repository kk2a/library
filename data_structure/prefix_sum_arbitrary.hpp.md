...
data:
  ...
  - code: "#ifndef KK2_DARA_STRUCTURE_PREFIX_SUM_ARBITRARY_HPP\n#define KK2_DARA_STRUCTURE_PREFIX_SUM_ARBITRARY_HPP\
      ...
      \ base(base_),\n          dim(int(base_.size())) {\n        inner_base.resize(dim\
      \ + 1, 1);\n        for (int i = 1; i < dim + 1; ++i) { inner_base[i] = inner_base[i\
      \ - 1] * base[i - 1]; }\n        n = 1;\n        for (int x : base) n *= x;\n\
      \        acc.resize(n, T{});\n    }\n\n    constexpr PrefixSumArbitrary(const\
      \ std::vector<int> &base_, const std::vector<T> &a)\n        : base(base_),\n\
      \          dim(int(base_.size())),\n          acc(a) {\n        inner_base.resize(dim\
      \ + 1, 1);\n        for (int i = 1; i < dim + 1; ++i) { inner_base[i] = inner_base[i\
      \ - 1] * base[i - 1]; }\n        n = 1;\n        for (int x : base) n *= x;\n\
      ...
      \ / inner_base[i + 1]) {\n                    acc[coord + inner_base[i]] +=\
      \ acc[coord];\n                }\n            }\n        }\n    }\n\n    constexpr\
      \ void init_set(const std::vector<int> &p, T x) {\n        assert(int(p.size())\
      \ == dim);\n        int coord = 0;\n        for (int i = 0; i < dim; ++i) {\n\
