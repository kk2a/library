...
data:
  ...
  - code: "#ifndef KK2_DATA_STRUCTURE_W_ARY_TREE_HPP\n#define KK2_DATA_STRUCTURE_W_ARY_TREE_HPP\
      ...
      \ x) const {\n        assert(0 <= x && x < _n);\n        return d.back()[x /\
      \ W] >> (x % W) & 1;\n    }\n\n    void insert(int x) {\n        assert(0 <=\
      \ x && x < _n);\n        for (int dep = (int)d.size() - 1; dep >= 0; --dep,\
      ...
      \ = (int)d.size() - 1;\n        for (;; --dep, x /= W) {\n            Uint mask\
      \ = up_mask(d[dep][x / W], x % W);\n            if (dep != (int)d.size() - 1)\
      \ mask &= ~(Uint(1) << (x % W));\n            int next = _msb(mask);\n     \
      \       if (next != -1) {\n                ++dep;\n                x = (x /\
      \ W) * W + next;\n                break;\n            }\n            if (dep\
      \ == 0) return std::nullopt;\n        }\n\n        for (; dep < (int)d.size();\
      \ x = x * W + _msb(d[dep][x]), ++dep) {}\n        return x;\n    }\n\n    //\
      \ return min y s.t. count(y) == 1 && y >= x\n    std::optional<int> successor(int\
      ...
      \     return x;\n    }\n\n  private:\n    static int _msb(Uint x) { return x\
      \ ? msb<Uint>(x) : -1; }\n\n    static int _lsb(Uint x) { return x ? lsb<Uint>(x)\
      \ : -1; }\n\n    static Uint up_mask(Uint x, int i) { return x & (((Uint(1)\
      \ << i) - 1) | (Uint(1) << i)); }\n\n    static Uint dw_mask(Uint x, int i)\
      \ { return x & ~((Uint(1) << i) - 1); }\n};\n\n} // namespace kk2\n\n#endif\
      \ // KK2_DATA_STRUCTURE_W_ARY_TREE_HPP\n"
    name: default
  - code: "#line 1 \"data_structure/w_ary_tree.hpp\"\n\n\n\n#include <algorithm>\n\
      #include <cassert>\n#include <optional>\n#include <string>\n#include <vector>\n\
