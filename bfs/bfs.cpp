...
// dancing_grid
vector<vector<vector<int>>> dancing_bfs(vector<vector<string>> &s, char start, const int &t, const int &ini, const string &wall = "#") {
    const int vx[] = {0, 1, 0, -1}, vy[] = {1, 0, -1, 0};
    const int h = size(s[0]), w = size(s[0][0]);
    vector<vector<vector<int>>> dist(t, vector<vector<int>>(h, vector<int>(w, ini)));
    ...
    while (!que.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            ny += vy[i], nx += vx[i];
            if (dist[(q[0] + 1) % t][ny][nx] != ini) continue;
